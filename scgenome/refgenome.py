...
class RefGenomeInfo(object):
    def __init__(self, version):
        if version == 'hg19':
            ...
            self.chromosome_length = pd.Series(read_chromosome_lengths(genome_fasta_index)).reindex(self.chromosomes).astype(float)
            self.chromosome_length.index.name = 'chr'

            self.chromosome_end = self.chromosome_length.cumsum()
            #self.chromosome_end = calculate_chr_end(self.chromosome_length, self.chromosomes)
            self.chromosome_start = self.chromosome_end.shift(1)
            self.chromosome_start[0] = 0
            self.chromosome_start = self.chromosome_start.astype(float)
...
#print(info.chromosome_info.chromosome_length)
#print(info.chromosome_info.chromosome_mid)
